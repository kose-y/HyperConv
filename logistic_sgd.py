...
class LogisticRegression(object):
    ...
    def negative_log_likelihood(self, y, pen=0.):
        ...
        print y.shape[0]
        return -T.mean(T.log(self.p_y_given_x)[T.arange(y.shape[0]), y]) + pen * T.sum(self.W**2)
...
def load_mat(datfile='KSC.mat',datvar='KSC',labelfile='KSC_int.mat',labelvar='KSC_gt', samplevar=None, clip=1300):
    ''' load the dataset
    ...
    if not samplevar:
        ...
        nzidx = scipy.io.loadmat(labelfile)[samplevar]
    mat_small = mat[nzidx[:,0], nzidx[:,1]]
...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1000,
    ...
    cost = classifier.negative_log_likelihood(y,pen)
...
def sgd_optimization_hyperspec(learning_rate = 0.00002, n_epochs = 10000, dataset = 'KSC', batch_size = 60, pen=0.0):
    datasets = load_mat()
    ...
    y = T.ivector('y') # 1D vector of int labels
    
    #construct logistic regression class
    # KSC image: 176 bands
    classifier = LogisticRegression(input=x, n_in = 176, n_out = 13)
    
    # the cost to minimize: -log likelihood
    
    cost = classifier.negative_log_likelihood(y,pen)
    
    #compile theano function taht computes the mistakes made by the model on a minibatch
    ...
    # compiling a Theano function 'train_model': returns cost, updates the parameter.
    train_model = theano.function(inputs=[index],
            outputs=cost,
            updates=updates,
            givens={
...
if __name__ == '__main__':
    numpy.random.seed(13574)
    if(len(sys.argv)<2): sgd_optimization_hyperspec()
    else: sgd_optimization_hyperspec(pen=float(sys.argv[1]))               
